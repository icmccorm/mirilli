...
class TBTagTransition:
    ...
    def exists(self):
        return self.action.exists() and self.start_state is not None and self.end_state is not None and self.kind is not None


class TBError:
    def __init__(self, action_type, is_child, accessed_tag_transition, conflicting_tag_transition):
...
def foreign_read(tb_action):
    return tb_action.action == TBOperation.read and tb_action.relation == TBHierarchy.foreign
def child_write(tb_action):
    return tb_action.action == TBOperation.write and tb_action.relation == TBHierarchy.child
def child_read(tb_action):
    return tb_action.action == TBOperation.read and tb_action.relation == TBHierarchy.child

def disabled_by(tb_reason, tb_transition):
    return tb_reason(tb_transition.action) and tb_transition.end_state == TBState.Disabled
...
def parse_tb_subtype(action, help_text):
    ...
    for line in help_text:
        ...
        if creation is not None:
            ...
            (referrent, tag, result_state, action_relation, action_type, retag_action_relation, retag_action_type) = destruction.groups()
            action_relation = action_relation if action_relation is not None else retag_action_relation

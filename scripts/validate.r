...
status_miri_comp <- read_csv(file.path("./data/results/stage2/status_miri_comp.csv"), col_names = c("crate_name", "version", "exit_code"), show_col_types = FALSE) %>%
    filter(exit_code != 0) %>%
    select(crate_name)

tests <- read_csv(file.path("./data/results/stage2/tests.csv"), col_names = c("exit_code", "has_ffi", "test_name", "crate_name"), show_col_types = FALSE) %>%
...
activated_zeroed <- read_activated("./data/results/stage3/zeroed")
activated_uninit <- read_activated("./data/results/stage3/uninit")
